...
def main():
    ...
    for label in [0, 12, 954]:
        ...
        save_img(image, f"./img_{label}_extra_credit.jpg")
        out = model(image)
...
def gradient_descent(img, model, loss, iterations=256):
    ...
    transform1 = transforms.GaussianBlur(3, 0.35)
    transform2 = transforms.GaussianBlur(3, 0.2)
    for _ in tqdm(range(iterations)):
        img.data = transform1(img.data)
        y_hat = model(img)
        loss_ = - loss(y_hat)
        if img.grad is not None:
            img.grad.data.zero_()
        loss_.backward()
        img.grad.data = transform2(img.grad.data)
        img.data = img.data - 0.005*img.grad.data - 0.000005*img.data
